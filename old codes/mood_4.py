...
LLM_MODEL_NAME = "qwen2.5:0.5b-instruct"  # Specified LLM model
...
def get_embedding(text, model=EMBED_MODEL):
    ...
    try:
        ...
        logger.debug(f"Received embedding for text: {text}")
        return embedding
    except requests.exceptions.RequestException as e:
        logger.error(f"Error fetching embedding: {e}")
        return None
    except (IndexError, KeyError) as e:
        logger.error(f"Unexpected response structure when fetching embedding: {e}")
        return None
        ...
        for utterance in asana.utterances:
            ...
            if utterance_embedding is None:
                ...
                if utterance_embedding is not None:
                    ...
                    logger.debug(f"Cached embedding for {asana.name}: '{utterance}'")
                else:
            ...
            logger.debug(f"Comparing with {asana.name}: Similarity = {similarity:.4f}")
...
def generate_final_comment(asana: YogaAsana, user_prompt: str):
    """
    Generates a concise final comment using the Ollama LLM Chat API based on the recommended Yoga Asana.
    """
    ...
    few_shot_examples = [
        ...
        {
            "role": "user",
            "content": "Recommended Asana: Downward Facing Dog (Adho Mukha Svanasana)\nHow to Do: Start on all fours with hands shoulder-width apart. Lift your hips towards the ceiling, forming an inverted V-shape. Keep your spine straight and heels reaching towards the mat.\nBenefits: Relieves stress, Invigorates the body, Stretches the hamstrings.\nGenerate a concise final comment."
        },
        {
            "role": "assistant",
            "content": "Incorporating Downward Facing Dog into your routine can significantly alleviate stress and invigorate your body. This pose not only stretches your hamstrings but also promotes overall flexibility and mental clarity."
        },
    ...
    # Construct the full message list with few-shot examples and the current request
    ...
    # Construct the payload for the Ollama Chat API
    payload = {
        "model": LLM_MODEL_NAME,
        "messages": messages,
        "stream": False  # Disable streaming for concise response
    }

    try:
        logger.debug(f"Sending request to Ollama Chat API with payload: {json.dumps(payload)}")
        response = requests.post(
            OLLAMA_CHAT_API_URL,
        ...
        logger.debug(f"Ollama Chat API Response: {json.dumps(response_json)}")

        # Extract the assistant's reply
        final_comment = response_json.get("message", {}).get("content", "").strip()
        logger.debug(f"Generated Final Comment: {final_comment}")
...
def generate_pdf_report(asana: YogaAsana, user_prompt: str, similarity: float, final_comment: str):
    """
    Generates a PDF report for the recommended Yoga Asana, including the final comment.
    """
    ...
    try:
        logger.debug(f"Generating PDF report at: {filepath}")
        ...
        c.drawCentredString(width / 2, top_margin, "Mood-Based Yoga Session Recommendation Report")

        # Date and User Prompt
        c.setFont("Helvetica", 12)
        c.drawString(left_margin, top_margin - 40, f"Date: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        c.drawString(left_margin, top_margin - 60, f"User Prompt: {user_prompt}")
        c.drawString(left_margin, top_margin - 80, f"Similarity Score: {similarity:.4f}")

        # Asana Name
        c.setFont("Helvetica-Bold", 16)
        c.drawString(left_margin, top_margin - 120, f"Recommended Asana: {asana.name}")

        # How to Do
        c.setFont("Helvetica-Bold", 14)
        c.drawString(left_margin, top_margin - 150, "How to Do:")
        c.setFont("Helvetica", 12)
        text = c.beginText(left_margin, top_margin - 170)
        ...
        # Frequency
        c.setFont("Helvetica-Bold", 14)
        c.drawString(left_margin, top_margin - 260, "Frequency:")
        c.setFont("Helvetica", 12)
        c.drawString(left_margin, top_margin - 280, asana.frequency)

        # Timing
        c.setFont("Helvetica-Bold", 14)
        c.drawString(left_margin, top_margin - 310, "Timing:")
        c.setFont("Helvetica", 12)
        c.drawString(left_margin, top_margin - 330, asana.timing)

        # Dietary Recommendations
        c.setFont("Helvetica-Bold", 14)
        c.drawString(left_margin, top_margin - 360, "Dietary Recommendations:")  
        c.setFont("Helvetica", 12)
        text = c.beginText(left_margin, top_margin - 390)
        ...
        c.setFont("Helvetica-Bold", 14)
        c.drawString(left_margin, top_margin - 430, "Lifestyle Recommendations:")  
        c.setFont("Helvetica", 12)
        text = c.beginText(left_margin, top_margin - 450)
        ...
        # Benefits
        c.setFont("Helvetica-Bold", 14)
        c.drawString(left_margin, top_margin - 500, "Benefits:")  # Added space above Benefits
        c.setFont("Helvetica", 12)
        text = c.beginText(left_margin, top_margin - 520)
        for line in asana.benefits.split(','):
            text.textLine(f"- {line.strip()}")
        c.drawText(text)

        # Final Comment from LLM
        final_comment_y = top_margin - 590  # Added more vertical space above
        c.setFont("Helvetica-Bold", 14)
        c.drawString(left_margin, final_comment_y, "Final Comment:")
        c.setFont("Helvetica", 12)
        ...
        # Define a frame for the final comment
        final_comment_y -= 10  # Added slight spacing before the paragraph starts
        final_comment_width = width - left_margin - right_margin
        final_comment_height = 60  # Reduced height to avoid too much white space

        frame = Frame(left_margin, final_comment_y - final_comment_height, final_comment_width, final_comment_height, showBoundary=0)
        para = Paragraph(final_comment, styleN)
        frame.addFromList([para], c)

        # Finalize PDF
        c.showPage()
        c.save()
        logger.debug(f"Generated PDF report: {filepath}")
        return filepath
...
def startup_event():
    ...
    """
    logger.info(f"Attempting to load embedding model '{EMBED_MODEL}'...")
...
async def process_prompt(prompt: Prompt):
    ...
    try:
        ...
        if prompt_embedding is None:
            logger.error("Failed to retrieve embedding for the prompt.")
        ...
        if best_asana is None or similarity < SIMILARITY_THRESHOLD:
            logger.info("No suitable Yoga Asana found for the given prompt.")
            return {
                "status": "no_match",
                "message": "No suitable Yoga Asana found for your current mood."
            }
        else:
            logger.info(f"Selected Yoga Asana: {best_asana.name} with similarity: {similarity:.4f}")

            # Generate final comment using Ollama LLM
            final_comment = generate_final_comment(best_asana, user_prompt)
            logger.debug(f"Generated Final Comment: {final_comment}")

            # Generate PDF report
            pdf_path = generate_pdf_report(best_asana, user_prompt, similarity, final_comment)
            if pdf_path is None:
            ...
            logger.info(f"Processed prompt in {response_time} seconds.")
            ...
            return {
                ...
                "final_comment": final_comment,  # Include the final comment in the response
...
async def download_report(report_filename: str):
    ...
    if os.path.exists(filepath):
        logger.info(f"Serving PDF report: {filepath}")
