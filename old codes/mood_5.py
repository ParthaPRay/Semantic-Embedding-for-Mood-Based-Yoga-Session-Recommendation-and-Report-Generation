...
CSV_COLUMNS = [
    ...
    "pdf_report_time", "network_latency"
]
...
def generate_final_comment(asana: YogaAsana, user_prompt: str):
    """
    Generates a concise final comment using the Ollama LLM Chat API and returns metrics from LLM response.
    Also measures network_latency for the request.
    """
    few_shot_examples = [
        {
            "role": "system",
    ...
    try:
        ...
        total_duration = response_json.get("total_duration", None)
        load_duration = response_json.get("load_duration", None)
        prompt_eval_count = response_json.get("prompt_eval_count", None)
        prompt_eval_duration = response_json.get("prompt_eval_duration", None)
        eval_count = response_json.get("eval_count", None)
        eval_duration = response_json.get("eval_duration", None)
...
def generate_pdf_report(asana: YogaAsana, user_prompt: str, similarity: float, final_comment: str):
    ...
    try:
        ...
        c.setFont("Helvetica-Bold", 14)
        c.drawString(left_margin, top_margin - 500, "Benefits:")
        c.setFont("Helvetica", 12)
        ...
        final_comment_y = top_margin - 590
        c.setFont("Helvetica-Bold", 14)
        ...
        styleN.leading = 14
        final_comment_y -= 10
        final_comment_width = width - left_margin - 50
        final_comment_height = 60
...
async def process_prompt(prompt: Prompt):
    ...
    try:
        ...
        if best_asana is None or similarity < SIMILARITY_THRESHOLD:
            ...
            if not llm_metrics:
                # In case no metrics, set defaults
                llm_metrics = {
                    "total_duration": None,
                    "load_duration": None,
            ...
            # Calculate tokens_per_second if eval_count and eval_duration are available
            ...
            log_data = {
                ...
                "total_duration": llm_metrics.get("total_duration"),
                "load_duration": llm_metrics.get("load_duration"),
                "prompt_eval_count": llm_metrics.get("prompt_eval_count"),
                "prompt_eval_duration": llm_metrics.get("prompt_eval_duration"),
                "eval_count": llm_metrics.get("eval_count"),
                "eval_duration": llm_metrics.get("eval_duration"),
                "tokens_per_second": tokens_per_second,
                "pdf_report_time": pdf_report_time,
                "network_latency": llm_metrics.get("network_latency")
            }
