# Yoga Asana Static Router using FastAPI and Cosine Similarity
# This application routes user mood prompts to the appropriate Yoga Asana based on semantic similarity.
# Debug messages are printed to trace the selection process and embedding model loading.
# Caching minimizes the time to deliver the exact what to do.
...
async def process_prompt(prompt: Prompt):
    """
    Processes the user's mood prompt and returns the recommended Yoga Asana.
    """
    ...
    try:
        ...
        if prompt_embedding is None:
            ...
            return {"status": "error", "message": "Failed to retrieve embedding for the prompt."}
